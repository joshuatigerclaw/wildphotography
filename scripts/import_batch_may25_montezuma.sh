...
# WildPhotography Import Batch - Montezuma (May 25)
# Uses pre-resolved gallery_id/gallery_slug from queue
# Processes from import_batch_active.json

set -e

QUEUE_FILE="/Users/joshuatenbrink/.openclaw/workspace/wildphotography/inventory/import_batch_active.json"
QUEUE_ITEMS_FILE="/tmp/pending_montezuma_batch.jsonl"
...
echo "Extracting pending items from import_batch_active.json..."
jq -c 'map(select(.status == "pending"))[0:100][]' "$QUEUE_FILE" > "$QUEUE_ITEMS_FILE"
...
while IFS= read -r ITEM; do
    ...
    GALLERY_ID=$(echo "$ITEM" | jq -r '.gallery_id')
    GALLERY_SLUG=$(echo "$ITEM" | jq -r '.gallery_slug')
    
    echo "[$COUNT] Processing: $FILENAME (Gallery: $GALLERY_SLUG, ID: $GALLERY_ID)"
    
    # Check if source file exists
    if [ ! -f "$SOURCE_PATH" ]; then
        echo "  - Source file not found: $SOURCE_PATH"
        FAILED=$((FAILED + 1))
        jq --argjson id "$ID" --argjson status "skipped" --arg reason "file_not_found" \
           '(.[] | select(.id == $id) | .status) = $status | (.[] | select(.id == $id) | .skip_reason) = $reason' \
           "$QUEUE_FILE" > /tmp/queue_updated.json && mv /tmp/queue_updated.json "$QUEUE_FILE"
        continue
    fi
    ...
    if [ "$EXISTS" -gt 0 ]; then
        ...
        jq --argjson id "$ID" --argjson status "skipped" --arg reason "duplicate" \
           '(.[] | select(.id == $id) | .status) = $status | (.[] | select(.id == $id) | .skip_reason) = $reason' \
           "$QUEUE_FILE" > /tmp/queue_updated.json && mv /tmp/queue_updated.json "$QUEUE_FILE"
        continue
    fi
    ...
    # Update queue status
    jq --argjson id "$ID" --argjson status "completed" \
...
echo ""
echo "=== IMPORT BATCH COMPLETE (Montezuma - May 25) ==="
...
echo "Failed: $FAILED"
echo "Gallery: $GALLERY_SLUG (ID: $GALLERY_ID)"
