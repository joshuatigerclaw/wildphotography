#!/usr/bin/env node
/**
 * fix-4-broken-derivatives.js
 * Targeted repair for photos 65796, 41256, 45064, 70566
 * Originals exist in R2, derivatives are missing.
 */
const { Pool } = require('pg');
const sharp = require('sharp');
const { S3Client, GetObjectCommand, PutObjectCommand, HeadObjectCommand } = require('@aws-sdk/client-s3');

const NEON_CONFIG = {
  host: 'ep-calm-fire-ad0dfnqd-pooler.c-2.us-east-1.aws.neon.tech',
  database: 'wildphotography',
  user: 'neondb_owner',
  password: 'npg_8MuC1tvKIOoj',
  ssl: { rejectUnauthorized: false },
  max: 3,
};

const R2_CONFIG = {
  endpoint: 'https://3ec62f93675c404fe4a9a4949e38e5e5.r2.cloudflarestorage.com',
  bucket: 'wildphoto-storage',
  accessKey: 'b821d56d29d9a2c716f783fc481e2f75',
  secretKey: '3af780dfe8dbb6d48b792e4bf8ba5836ae659c89192645a7ae971300464aa48f',
  publicUrl: 'https://images.wildphotography.com',
};

const SIZES = {
  thumb: { width: 400, quality: 80 },
  small: { width: 900, quality: 85 },
  medium: { width: 1600, quality: 85 },
  large: { width: 2400, quality: 90 },
  preview: { width: 1600, quality: 85 },
};

const dbPool = new Pool(NEON_CONFIG);
const s3 = new S3Client({
  endpoint: R2_CONFIG.endpoint,
  region: 'auto',
  credentials: { accessKeyId: R2_CONFIG.accessKey, secretAccessKey: R2_CONFIG.secretKey },
});

async function getObject(key) {
  try {
    const cmd = new GetObjectCommand({ Bucket: R2_CONFIG.bucket, Key: key });
    const resp = await s3.send(cmd);
    if (!resp.Body) return null;
    const chunks = [];
    for await (const chunk of resp.Body) {
      chunks.push(chunk);
    }
    return Buffer.concat(chunks);
  } catch (e) {
    console.log(`    getObject error for ${key}: ${e.message}`);
    return null;
  }
}

async function objectExists(key) {
  try {
    const cmd = new HeadObjectCommand({ Bucket: R2_CONFIG.bucket, Key: key });
    await s3.send(cmd);
    return true;
  } catch {
    return false;
  }
}

async function putObject(key, data, contentType = 'image/webp') {
  try {
    const cmd = new PutObjectCommand({ Bucket: R2_CONFIG.bucket, Key: key, Body: data, ContentType: contentType });
    await s3.send(cmd);
    return true;
  } catch (e) {
    console.log(`    putObject error: ${e.message}`);
    return false;
  }
}

async function fixPhoto(photo_id) {
  console.log(`\n[${photo_id}] Starting repair`);

  // Fetch photo details
  const photoResult = await dbPool.query(
    `SELECT id, slug, original_r2_key FROM photos WHERE id = $1`,
    [photo_id]
  );

  if (photoResult.rows.length === 0) {
    console.log(`  ERROR: photo not found`);
    return false;
  }

  const photo = photoResult.rows[0];
  console.log(`  slug: ${photo.slug}`);
  console.log(`  original_r2_key: ${photo.original_r2_key}`);

  if (!photo.original_r2_key) {
    console.log(`  ERROR: no original_r2_key`);
    return false;
  }

  // Get original from R2
  const originalData = await getObject(photo.original_r2_key);
  if (!originalData) {
    console.log(`  ERROR: original not found in R2 at ${photo.original_r2_key}`);
    return false;
  }
  console.log(`  Original size: ${originalData.length} bytes`);

  const slugSlug = photo.slug.replace(/[^a-z0-9-]/gi, '-');
  const derivatives = {};
  let generated = 0;
  let failed = 0;

  for (const [sizeName, config] of Object.entries(SIZES)) {
    const r2Key = `derivatives/${sizeName}/${slugSlug}-${sizeName}.webp`;

    // Check if already exists
    const exists = await objectExists(r2Key);
    if (exists) {
      derivatives[sizeName] = { r2_key: r2Key, public_url: `${R2_CONFIG.publicUrl}/${r2Key}` };
      generated++;
      console.log(`    [${sizeName}] Already exists, skipping`);
      continue;
    }

    try {
      let buffer;
      if (sizeName === 'preview') {
        buffer = await sharp(originalData)
          .resize(config.width, null, { withoutEnlargement: true })
          .jpeg({ quality: config.quality })
          .toBuffer();
      } else {
        buffer = await sharp(originalData)
          .resize(config.width, null, { withoutEnlargement: true })
          .webp({ quality: config.quality })
          .toBuffer();
      }

      const ok = await putObject(r2Key, buffer, sizeName === 'preview' ? 'image/jpeg' : 'image/webp');
      if (ok) {
        derivatives[sizeName] = { r2_key: r2Key, public_url: `${R2_CONFIG.publicUrl}/${r2Key}` };
        generated++;
        console.log(`    [${sizeName}] Generated and uploaded (${buffer.length} bytes) -> ${r2Key}`);
      } else {
        failed++;
        console.log(`    [${sizeName}] FAILED to upload`);
      }
    } catch (e) {
      failed++;
      console.log(`    [${sizeName}] Sharp error: ${e.message}`);
    }
  }

  console.log(`  Generated: ${generated}, Failed: ${failed}`);

  // Update photos table
  if (generated > 0) {
    try {
      await dbPool.query(`
        UPDATE photos SET
          thumb_url = COALESCE($1, thumb_url),
          small_url = COALESCE($2, small_url),
          medium_url = COALESCE($3, medium_url),
          large_url = COALESCE($4, large_url),
          preview_url = COALESCE($5, preview_url),
          r2_thumb_key = COALESCE($6, r2_thumb_key),
          r2_web_small_key = COALESCE($7, r2_web_small_key),
          r2_web_large_key = COALESCE($8, r2_web_large_key),
          derivatives_complete = true,
          ready_for_public_render = true,
          search_ready = true,
          updated_at = NOW()
        WHERE id = $9
      `, [
        derivatives['thumb']?.public_url || null,
        derivatives['small']?.public_url || null,
        derivatives['medium']?.public_url || null,
        derivatives['large']?.public_url || null,
        derivatives['preview']?.public_url || null,
        derivatives['thumb']?.r2_key || null,
        derivatives['small']?.r2_key || null,
        derivatives['large']?.r2_key || null,
        photo_id,
      ]);
      console.log(`  DB updated successfully`);
    } catch (e) {
      console.log(`  DB update error: ${e.message}`);
    }
  }

  // Update queue
  const newStatus = (generated > 0 && failed === 0) ? 'completed' : (generated === 0 && failed > 0) ? 'permanently_failed' : 'partial';
  await dbPool.query(
    `UPDATE derivative_rebuild_queue SET status = $1, resolution = $2, date_modified = NOW() WHERE photo_id = $3`,
    [newStatus, `${generated} generated, ${failed} failed`, photo_id]
  );

  return generated > 0 && failed === 0;
}

async function main() {
  const targetIds = [65796, 41256, 45064, 70566];
  console.log(`=== WildPhotography 4-Photo Derivative Fix ===`);
  console.log(`Started: ${new Date().toISOString()}`);

  let success = 0;
  for (const id of targetIds) {
    const ok = await fixPhoto(id);
    if (ok) success++;
  }

  await dbPool.end();
  console.log(`\n=== DONE: ${success}/${targetIds.length} fixed ===`);
}

main().catch(console.error);
