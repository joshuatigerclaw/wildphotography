#!/usr/bin/env python3
"""
WildPhotography Batch 164
Stock-Photos/Marine-Life: 29 new photos found.
Gallery: Marine-Life (58)
"""
...
GALLERY_MAP = {
    "Stock-Photos-of-the-World/Marine-Life": (58, "marine-life"),
}
...
temp_dir = "/tmp/wildphoto_batch164"
...
print("=== WildPhotography Batch 164 ===")
...
for folder_key, (gallery_id, gallery_slug) in GALLERY_MAP.items():
    ...
    if not is_active:
        print(f"SKIP (inactive Neon gallery): {folder_name}")
        stats['folders_skipped_no_existing_gallery'].append(folder_key)
        continue
    ...
    for fpath, fname, content_hash, fsize in new_files[:100]:
        try:
            ...
            slug = f"{gallery_slug}-{content_hash[:16]}"
            ...
            if upload_failures >= 3:
                print(f"    STOPPING: 3 consecutive upload failures")
                break
