#!/usr/bin/env python3
"""
WildPhotography Batch 173
Scan folders: favorites
Note: favorites does not map to an existing gallery - will be skipped
Source: /Volumes/ADATA SC740/Smugmug Backup/_uploaded_mirror/originals
"""
...
BASE = "/Volumes/ADATA SC740/Smugmug Backup/_uploaded_mirror/originals"

# favorites has no existing gallery - all photos will be skipped
GALLERY_MAP = {
    "favorites": None,  # No existing gallery match
}
...
temp_dir = "/tmp/wildphoto_batch173"
...
print("=== WildPhotography Batch 173 ===")
...
for folder_key, gallery_info in GALLERY_MAP.items():
    ...
    stats['per_gallery'][folder_name] = {
        ...
        'gallery_id': gallery_info[0] if gallery_info else None, 
        'gallery_slug': gallery_info[1] if gallery_info else None
    }
    ...
    # Check if folder maps to an existing gallery
    if gallery_info is None:
        print(f"SKIPPED: {folder_name} → no existing gallery match")
        stats['folders_skipped_no_existing_gallery'].append(folder_key)
        stats['per_gallery'][folder_name]['status'] = 'no_existing_gallery'
        continue

    gallery_id, gallery_slug = gallery_info
