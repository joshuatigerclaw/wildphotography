#!/usr/bin/env python3
"""
WildPhotography Batch 150
Scan 5 galleries never before imported, import only confirmed new files.
Galleries: Birds(5), Isla-Tortuga(47), Best-of-Costa-Rica(19), 
           Montezuma-Costa-Rica(60), Flowers-plants-trees(35)
"""
...
# Format: (folder_key, gallery_id, gallery_slug)
GALLERY_MAP = {
...
temp_dir = "/tmp/wildphoto_batch150"
...
print("=== WildPhotography Batch 150 ===")
...
for folder_key, (gallery_id, gallery_slug) in GALLERY_MAP.items():
    ...
    stats['per_gallery'][folder_name] = {'total_on_disk': 0, 'new': 0, 'duplicates': 0, 'status': 'unknown'}
    ...
    all_files = sorted([f for f in os.listdir(folder_path)
                        if os.path.isfile(os.path.join(folder_path, f)) and f.lower().endswith(EXTENSIONS) and not f.startswith('._')])
...
for g, v in stats['per_gallery'].items():
    print(f"  {g}: total_on_disk={v['total_on_disk']} new={v['new']} duplicates={v['duplicates']} status={v['status']}")
