...
function buildPhotoDescription(photo: FeedPhoto, siteUrl: string): string {
  ...
  if (imageUrl) parts.push(`<img src="${escapeXml(imageUrl)}" alt="${escapeXml(buildPhotoTitle(photo))}" />`);
...
export async function GET(): Promise<Response> {
  const siteUrl = (process.env.SITE_URL || 'https://wildphotography.com').replace(/\/+$/, '');
  const now = new Date();
  ...
  try {
    ...
    console.error('[feed/daily-random] DB error:', err);
