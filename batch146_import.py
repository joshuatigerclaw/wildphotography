#!/usr/bin/env python3
"""
WildPhotography Batch 146 - Continuation from Batch 145
Scan 8 remaining galleries from batch 145, import only confirmed new files.
"""
import os
import json
import hashlib
...
# Galleries and their Neon IDs from batch 145 continuation
# Key = subfolder name under Costa-Rica-Gallery
GALLERY_MAP = {
    "Costa-Rica-Gallery/Peninsula-Papagayo":          (125, "costa-rica-gallery-peninsula-papagayo"),
    ...
    "Costa-Rica-Gallery/Poas-Volcano-Costa-Rica":    (162, "poas-volcano"),
...
def scan_folder(folder_name, gallery_id, gallery_slug):
    ...
    for fname in sorted(os.listdir(folder_path)):
        ...
        if not os.path.isfile(fpath) or not fname.lower().endswith(('.jpg','.jpeg','.png','.tiff','.webp')):
            continue
        ...
        if content_hash not in existing:
            new_items.append({
                'id': f"batch146_{gallery_slug}_{content_hash[:12]}",
                'type': 'photo',
                'source_path': fpath,
            ...
            existing.add(content_hash)  # prevent duplicates within folder

    return new_items

print("=== WildPhotography Batch 146 ===")
...
temp_dir = "/tmp/wildphoto_batch146"
...
for folder_key, (gallery_id, gallery_slug) in GALLERY_MAP.items():
    ...
    if not os.path.isdir(folder_path):
        print(f"SKIP: {folder_name} → not found on disk")
        continue

    print(f"Scanning: {folder_name} (gallery {gallery_id})")
    existing = get_existing_hashes(gallery_id)
    ...
    # Process up to 100 photos total (per batch constraint)
