#!/usr/bin/env python3
"""
WildPhotography Import Manager - Peru Remainder Batch Builder
Processes: Peru (254 remaining after 28 done in round 2)
Limit: 100 photos
"""
import os
import json
...
def main():
    ...
    print("Building Peru remainder import batch (20260328-03)...")
    ...
    print(f"Peru folder: {len(image_files)} total files")
    ...
    print(f"New files in Peru: {len(all_new)}")
    ...
    for i, (source_path, filename, content_hash) in enumerate(batch_items):
        item = {
            'id': f'peru3_20260328_{content_hash[:12]}',
            'type': 'photo',
    ...
    meta = {
        ...
        'batch_type': 'south_america_peru_remainder',
        'created_at': ts,
        'status': 'ready_to_run',
        'note': f'Peru remainder batch (20260328-03). {len(items)} items this batch. {len(remaining)} overflow for next batch.',
