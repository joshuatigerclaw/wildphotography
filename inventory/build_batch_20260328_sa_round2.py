#!/usr/bin/env python3
"""
WildPhotography Import Manager - South-America Batch Builder (Round 2)
Processes: Colombia (53), Panama (19), Peru (282)
Limit: 100 photos
"""
import os
import json
...
def main():
    ...
    print("Building South-America import batch (20260328-02)...")
    ...
    all_new = []
    folder_results = {}
    folders = ['Colombia-Cartagena-Islas-Baru', 'Panama-', 'Peru']
    ...
    for i, (source_path, filename, content_hash) in enumerate(batch_items):
        ...
        gallery_id, gallery_slug = GALLERY_MAP.get(folder_name, (118, 'south-america-colombia-cartagena-islas-baru'))
        
        item = {
            'id': f'sa2_20260328_{content_hash[:12]}',
            'type': 'photo',
    ...
    meta = {
        ...
        'batch_type': 'south_america_round2',
        'created_at': ts,
        'status': 'ready_to_run',
        'note': f'South-America batch (20260328-02). Colombia/Panama/Peru: {len(items)} items this batch. {len(remaining)} overflow.',
        'batch_folders_remaining': remaining_info,
        'folders_processed': 0,
        'folders_in_batch': folders,
