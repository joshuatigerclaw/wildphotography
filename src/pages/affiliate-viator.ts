/**
 * Viator Affiliate Redirect Page
 * Route: /go/viator/:slug → redirect to real Viator URL
 */

import type { Env } from "../types";

export async function renderViatorRedirect(slug: string, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (offer) {
      const redirectUrl = offer.affiliate_redirect_url || offer.pretty_link || `https://www.viator.com/P00094502/tours/Costa-Rica`;
      const stars = offer.rating_if_available ? "⭐".repeat(Math.round(parseFloat(offer.rating_if_available))) : "";

      const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Redirecting to Viator: ${(offer.title || '').replace(/</g, '&lt;')} | WildPhotography</title>
<meta name="description" content="Book ${(offer.title || 'this accommodation').replace(/</g, '&lt;')} — recommended by WildPhotography.">
<style>
  body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;background:#fff5f0}
  .card{background:white;border-radius:16px;padding:2.5rem;max-width:500px;width:92%;text-align:center;box-shadow:0 4px 32px rgba(192,86,33,0.12)}
  .badge{display:inline-block;background:#c05621;color:white;padding:0.3rem 0.9rem;border-radius:20px;font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:1.2rem}
  ...
  .meta-item{font-size:0.85rem;color:#4a5568;background:#fff5f0;padding:0.4rem 0.8rem;border-radius:6px}
  .cta{display:inline-block;background:#c05621;color:white;padding:0.85rem 2.2rem;border-radius:10px;text-decoration:none;font-weight:700;font-size:1.05rem;transition:background 0.2s;box-shadow:0 2px 8px rgba(192,86,33,0.3)}
  .cta:hover{background:#9c3d10;text-decoration:none}
  ...
  .bar-fill{height:100%;background:linear-gradient(90deg,#c05621,#dd6b20);width:0;animation:load 1.8s ease forwards}
...
<div class="card">
  ...
  <h1>${offer.title || 'Book Your Costa Rica Stay'}</h1>
  ...
  <a href="${redirectUrl}" class="cta">Book on Viator →</a>
  ...
  <p class="disclaimer">Redirecting in 2 seconds... <a href="${redirectUrl}" style="color:#c05621">Click here if not redirected</a></p>
</div>
</body></html>`;
    ...
    console.error("[Viator redirect] DB error:", e);
  }

  // 404 fallback
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Booking Not Found | WildPhotography</title>
<style>
  ...
  a{color:#c05621;font-weight:600;text-decoration:none}
</style>
</head>
<body>
<div class="card">
