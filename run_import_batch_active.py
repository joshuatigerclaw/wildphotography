#!/usr/bin/env python3
"""
WildPhotography Import Worker - Run next batch
"""
...
RESULT_PATH = "/Users/joshuatenbrink/.openclaw/workspace/wildphotography/inventory/fresh_batch_next_5_results.json"
...
GALLERY_MAP = {
    ...
    "Tambor-Photos": (96, "tambor-photos"),
...
# Group by folder (up to 5 folders)
folders_seen = {}
for item in items:
...
print(f"Folders to process: {len(folder_list)}")
...
for folder_name in folder_list:
    folder_items = folders_seen[folder_name][:100]
    if folder_name not in GALLERY_MAP:
        print(f"SKIPPED: {folder_name} → no existing gallery match")
        stats["skipped_dup"] += len(folder_items)
        for item in folder_items:
            results.append({"id": item.get("id"), "status": "skipped_no_gallery", "gallery_folder": folder_name})
        continue
    
    gallery_id, gallery_slug = GALLERY_MAP[folder_name]
    print(f"\nProcessing folder: {folder_name} → gallery_id={gallery_id} slug={gallery_slug}")
    
    for i, item in enumerate(folder_items):
        if stats["imported"] >= 100:
            print(f"  Max 100 photos reached, stopping.")
            break
        ...
        if not source_path or not content_hash:
            results.append({"id": item.get("id"), "status": "skipped_missing_data", "gallery_folder": folder_name})
            stats["skipped_dup"] += 1
            continue
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                """, (
                    ...
                    True, True,
                    True, source_path, "import_batch_active"
                ))
            ...
            stats["failed"] += 1
            failure_log.append({"filename": filename, "error": err_str})
...
processed_ids = set(r["id"] for r in results if r["status"] in ("ok", "skipped_dup"))
...
output = {
    ...
    "total_items_in_queue": len(items),
    "total_photos_processed": stats["total"],
    "imported": stats["imported"],
    "skipped_dup": stats["skipped_dup"],
    "failed": stats["failed"],
    "results": results[:100],
    "failure_log": failure_log[:20]
}
